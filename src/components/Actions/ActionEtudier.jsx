...
export function ActionEtudier({ onClose, onMarkUsed, onOpenInnovations }) {
  ...
  if (confirmed) {
    ...
    const nb = population[chosen] || 0
    return (
      <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', zIndex:850,
        display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
        <div style={{ background:'white', borderRadius:14, width:'100%', maxWidth:380,
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', zIndex:850,
      display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
    <div style={{ background:'white', borderRadius:14, width:'100%', maxWidth:380,
      boxShadow:'0 16px 48px rgba(0,0,0,.3)', padding:'20px', display:'flex', flexDirection:'column', gap:10 }}>
      <div style={{ fontWeight:600, fontSize:15, color:'#1e293b', marginBottom:4 }}>📚 Action Étudier</div>
      ...
      {POP_TYPES.map(pt => {
        ...
        return (
          <button key={pt.key} onClick={() => setChosen(pt.key)}
            disabled={false}
            style={{
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:12, color: nb===0 ? '#94a3b8' : '#64748b' }}>
                {nb} {pt.label.toLowerCase()} → {nb > 0 ? `${nb} jeton${nb>1?'s':''} ${pt.token}` : '0 jeton (confirmer quand même)'}
              </div>
            </div>
